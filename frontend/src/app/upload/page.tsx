...
const UPLOAD_CLIENT_VERSION = "upload-next-api-v2";
...
export default function UploadPage() {
  ...
  useEffect(() => {
    console.log(`[MEMORIA upload] loaded ${UPLOAD_CLIENT_VERSION}`);
  }, []);
  ...
  useEffect(() => {
    ...
    const stepTimer = setInterval(() => {
      setStepIndex((i) =>
        Math.min(i + 1, ANALYSIS_STEPS.length - 1),
      );
    }, 720);
  ...
  const handleAnalyze = async () => {
    ...
    try {
      // 認証トークンの付与はNext.jsのAPIルート側で行い、画面側はFormDataだけ送る。
      console.log(`[MEMORIA upload] submit via ${UPLOAD_CLIENT_VERSION}`);
      ...
      alert(
        error instanceof Error
          ? error.message
          : "AI解析中にエラーが発生しました。バックエンドが動いているか確認してください。",
      );
    } finally {
          ...
          <GlassCard className="w-full max-w-sm border border-white/45 bg-mono-paper/88 p-4 shadow-elev sm:p-5">
            <div className="flex items-start justify-between gap-3">
              ...
              <div
                ...
                style={{
                  background: `conic-gradient(var(--primary) ${Math.round(
                    progressPct,
                        ...
                        >
                          <span
                            className={cn(
                              ...
                              current &&
                                "bg-mono-paper ring-2 ring-primary",
                              !done &&
                                !current &&
