...
`frontend/src/lib/mood.ts` の `MOODS` と一致（3D は `public/models/animals/*.glb`）。

| 感情状態（label） | 絵文字 | 3D（ファイル） | キャッチコピー（tagline） |
|------------------|--------|----------------|---------------------------|
| 自由っぽい | 🐱 | `cat.glb` | 気まぐれに世界を歩く |
| 穏やか | 🐻 | `bear.glb` | どっしり構えた安心感 |
| 好奇心旺盛 | 🦊 | `fox.glb` | 知りたがりの探究者 |
| 寂しい・不安 | 🐭 | `rat.glb` | そっと寄り添う、繊細な心 |
| 親しみ | 🐶 | `dog.glb` | いつも隣にいる安心 |
| 愛らしい | 🐧 | `penguin.glb` | みんなと過ごす幸せ |
